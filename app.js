...
// const saltRounds = 10; // Jumlah putaran salt untuk enkripsi password
...
server.route({
   ...
   handler: async (request, h) => {
      ...
      // Query untuk mendapatkan total profit dari tabel total_profit
      const getProfitQuery = 'SELECT * FROM total_profit WHERE user_id = ?';

      try {
         const profitResults = await new Promise((resolve, reject) => {
         ...
         if (profitResults.length === 0) {
            return h.response({ status: 'success', message: 'Total profit not found', totalProfit: 0 }).code(200);
         }

         const totalProfit = profitResults[0].total_profit;

         return h.response({ status: 'success', message: 'Total profit retrieved successfully', totalProfit }).code(200);
      } catch (error) {
         console.error('Error:', error);
...
// Pastikan Anda telah mengimpor modul yang diperlukan dan membuat koneksi ke database

// Route untuk mengupdate total profit
